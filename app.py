...
def edit_post(post_id):
    ...
    # Kiểm tra quyền: owner, admin, hoặc user là collaborator/editor
    if not (post.user_id == current_user.id or 
...
def delete_post(post_id):
    ...
    # Kiểm tra quyền: chỉ owner, admin, hoặc user là editor mới được xóa
    if not (post.user_id == current_user.id or 
            current_user.is_admin or 
...
def bulk_delete_posts():
    ...
    for post_id in post_ids:
        ...
        if post:
            # Kiểm tra quyền xóa (owner, admin hoặc editor)
            if post.user_id == current_user.id or current_user.is_admin or current_user.is_editor():
                # Xóa file ảnh nếu có
                if post.image_file:
                    try:
                        os.remove(os.path.join(app.root_path, app.config['UPLOAD_FOLDER'], post.image_file))
...
def admin_change_role(user_id, role):
    ...
    if role not in ['viewer', 'collaborator', 'editor']:
        flash('Role không hợp lệ!', 'danger')
        return redirect(url_for('admin_users'))
    
    user = User.query.get_or_404(user_id)
    
    # Không được thay đổi role của admin
    if user.is_admin:
        flash('Không thể thay đổi role của admin!', 'danger')
        return redirect(url_for('admin_users'))
    
    user.role = role
    db.session.commit()
    
    flash(f'Role của {user.username} đã được đổi thành {role}.', 'success')
    return redirect(url_for('admin_users'))
...
if __name__ == '__main__':
    with app.app_context():
        ...
        if 'role' not in [c['name'] for c in inspector.get_columns('user')]:
            db.engine.execute('ALTER TABLE user ADD COLUMN role VARCHAR(20) NOT NULL DEFAULT "viewer"')
